...
#include "funny_delay.h"

block_t osc_block __attribute__((restrict));

delay_engine::delay_engine( jack_port_t *in_port, jack_port_t *midi_p, jack_nframes_t nframes )
{
...
void 
delay_engine::dump_params() {
    params.dump();
}

paramMap &
delay_engine::get_params() {
    return params;
}

void 
delay_engine::fill_channel( float * __restrict__ buf, jack_nframes_t nframes )
{
    int i;
...
{
    return new delay_engine( in_port, midi_p, nframes );
}
